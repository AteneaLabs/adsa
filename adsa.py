...
output_file = "execution_logs.txt"
...
def azure_openai_request(messages):
    url = f"{AZURE_CONFIG['endpoint']}/openai/deployments/{AZURE_CONFIG['deployment_name']}/chat/completions?api-version={AZURE_CONFIG['api_version']}"
    ...
    try:
        ...
        print(colored(f"Error en Azure OpenAI request: {str(e)}", "red"))
        return None
...
def get_timeout_improvement(code, train_sample, problem_info, execution_time, max_runtime):
...
Return ONLY the complete optimized code wrapped in ```python and ``` markers."""
        ...
        return azure_openai_request(messages)
    except Exception as e:
...
def get_claude_fix(error_message, code, train_sample, problem_info):
...
with open('problem_info.md', 'r', encoding='utf-8') as file:
    problem_info = file.read()
...
for i in range(ITERATIONS):
    ...
    # Move old solution, progress report, and results to older_solutions folder
    if i > 0:
...
8. make sure that the script is creating and saving a progress_report.json file with the accuracy and other useful information as well as the results file to a file called results.csv
...
- save the progress report and the final accuracy to a file called progress_report.json as well as the results file to a file called results.csv
7. Prepare the results file in the required format
...
4. Model parameters and architecture"""}
    ...
    response = azure_openai_request(messages)
    ...
    while has_error:  # Keep trying with Claude until no errors
        ...
        if stderr:
            ...
            if is_actual_error(stderr):
                ...
                fixed_code = get_claude_fix(stderr, current_code, train_sample, problem_info)
...
print(colored("All solutions, progress reports, and resultss have been saved with iteration numbers in the older_solutions directory.", "green"))
